...
<body>
   ...
   <?php 
    ...
    <script>
        ...
        // Function to fetch and update results without page reload
        function fetchResults(searchValue) {
            ...
            // Fetch results using fetch API
            fetch(url)
                .then(response => response.text())
                .then(html => {
                    // Extract just the manhwa grid/list content
                    ...
                    if (emptyState) {
                        // If there's an empty state in the response
        ...
        // Function to handle reading status changes
        ...
        // Add event listeners to reading status selects
        document.querySelectorAll('.reading-status-select').forEach(select => {
            select.addEventListener('change', handleReadingStatusChange);
        });
        ...
        function addRereadButtons() {
            ...
            document.querySelectorAll('.manhwa-grid .manhwa-card').forEach(card => {
                ...
                if (readingStatus === 'Done') {
                    ...
                    // Remove button if status is not Done
