    ...
    <main>
        ...
        <div class="cover-slideshow">
            ...
            while ($slide = mysqli_fetch_assoc($slideshow_result)): 
                ...
                $quote = !empty($quotes) ? $quotes[$i % count($quotes)] : "";
            ?>
        ...
        <div class="main-content">
            ...
            if ($hour >= 5 && $hour < 12) {
                $message = $morningQuotes[array_rand($morningQuotes)];
            } else if ($hour >= 18 && $hour <= 21) {
                $message = $eveningQuotes[array_rand($eveningQuotes)];
            } else if ($hour >= 22 || $hour < 5) {
                $message = $midnightQuotes[array_rand($midnightQuotes)];
            } else {
                $message = "Track your manhwa collection";
            }
            ...
            .then(data => {
                if (data.success) {
                    if (action === 'mark_done') {
                        ...
                        // Update the reading status without page refresh
                        const card = event.target.closest('.manhwa-card');
                        if (card) {
                            // Update the plan to read count
