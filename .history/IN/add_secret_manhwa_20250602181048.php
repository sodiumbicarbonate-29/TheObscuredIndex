...
// Get manhwas from main library that aren't in secret shelf
$query = "SELECT m.* FROM Manhwas m
          WHERE m.title NOT IN (SELECT title FROM secret__manhwas WHERE user_id = ?)
    ...
    if (isset($_POST['add_to_secret'])) {
        ...
        if (empty($manhwa_ids)) {
            ...
            foreach ($manhwa_ids as $manhwa_id) {
                // Get the manhwa details from the main library
                $get_manhwa = "SELECT * FROM Manhwas WHERE manhwa_id = ?";
                $get_stmt = mysqli_prepare($conn, $get_manhwa);
                mysqli_stmt_bind_param($get_stmt, "i", $manhwa_id);
                mysqli_stmt_execute($get_stmt);
                $manhwa_result = mysqli_stmt_get_result($get_stmt);
                
                if ($manhwa = mysqli_fetch_assoc($manhwa_result)) {
                    // Check if this manhwa is already in the secret shelf
                    $check_query = "SELECT * FROM secret__manhwas WHERE user_id = ? AND title = ?";
                    $check_stmt = mysqli_prepare($conn, $check_query);
                    mysqli_stmt_bind_param($check_stmt, "is", $user_id, $manhwa['title']);
                    mysqli_stmt_execute($check_stmt);
                    $check_result = mysqli_stmt_get_result($check_stmt);
                    
                    if (mysqli_num_rows($check_result) == 0) {
                        // Insert the manhwa into the secret shelf
                        $insert_query = "INSERT INTO secret__manhwas (title, author, status, genre, description, reading_link, cover_image, upload_date, user_id) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?)";
        ...
        // Handle adding a new manhwa directly to secret shelf
        ...
        if (empty($title)) {
            ...
            if (empty($error)) {
                // Insert directly into secret__manhwas table
                $insert_secret = "INSERT INTO secret__manhwas (title, author, status, genre, description, reading_link, cover_image, upload_date, user_id) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?)";
                $secret_stmt = mysqli_prepare($conn, $insert_secret);
                mysqli_stmt_bind_param($secret_stmt, "sssssssi", $title, $author, $status, $genre, $description, $reading_link, $cover_image, $user_id);
                
                if (mysqli_stmt_execute($secret_stmt)) {
