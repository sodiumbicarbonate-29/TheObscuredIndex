...
// Even if user has access in database, they need to login each time
// Clear any existing secret shelf session
unset($_SESSION['secret_shelf_access']);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $password = isset($_POST['password']) ? $_POST['password'] : '';
    
    if ($password === 'dinnesh' || $password === 'noshamezone') {
        $insert_query = "INSERT INTO Secret_Shelf_Access (user_id, granted_date) VALUES (?, NOW())";
        $insert_stmt = mysqli_prepare($conn, $insert_query);
        mysqli_stmt_bind_param($insert_stmt, "i", $user_id);
        
        if (mysqli_stmt_execute($insert_stmt)) {
            // Set session variable to indicate active secret shelf access
            $_SESSION['secret_shelf_access'] = true;
            header("Location: secret-shelf.php");
            exit();
        } else {
            $error_message = "Error granting access. Please try again.";
        }
    } else {
...
<head>
    ...
    <title>Secret Shelf - Access Required</title>
