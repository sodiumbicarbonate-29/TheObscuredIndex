...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'update') {
    ...
    if (isset($_POST['reading_status']) && !empty($_POST['reading_status'])) {
        ...
        if ($reading_status === 'Reread') {
            ...
            $insert_reread = "INSERT INTO Reread_History (user_id, manhwa_id, start_date, created_at) 
                             VALUES (?, ?, ?, ?)";
            $reread_insert_stmt = mysqli_prepare($conn, $insert_reread);
            if ($reread_insert_stmt) {
                mysqli_stmt_bind_param($reread_insert_stmt, "iiss", $user_id, $manhwa_id, $reread_start, $current_time);
                if (!mysqli_stmt_execute($reread_insert_stmt)) {
                    $error .= " Error adding reread history: " . mysqli_error($conn);
                    error_log("Error adding reread history: " . mysqli_error($conn));
                }
                mysqli_stmt_close($reread_insert_stmt);
            } else {
                $error .= " Failed to prepare reread history statement: " . mysqli_error($conn);
                error_log("Failed to prepare reread history statement: " . mysqli_error($conn));
            }
        } 
...
try {
    ...
    if ($reread_stmt) {
        ...
        echo "<!-- Reread history count: " . mysqli_num_rows($reread_result) . " -->";
    }
} catch (Exception $e) {
    $reread_result = false;
    echo "<!-- Reread history error: " . $e->getMessage() . " -->";
}
                ...
                <div class="manhwa-info">
                    ...
                    <div class="reading-status">
                        ...
                        <div class="reading-actions">
                            ...
                            <?php if ($reread_result && mysqli_num_rows($reread_result) > 0): ?>
                            <button type="button" id="toggle-history-btn" class="read-btn history-btn">
    ...
    <script>
        ...
        if (toggleBtn) {
            toggleBtn.addEventListener('click', function() {
                ...
                if (rereadHistoryContent.style.display === 'none' || !rereadHistoryContent.style.display) {
